...
export default new Vuex.Store({
  ...
  actions: {
    ...
    fetchEmployees({ commit }, [department_id, page, perPage]) {
      ...
      page = page || 0;
      return window.axios
        .get(API_URL + "employees/" + department_id + "/?_page=" + page + "&_per_page=" + perPage)
        .then(response => {
          let data = response.data;
    ...
    fetchDocuments({ commit }, [category_id, page, perPage]) {
      ...
      page = page || 0;
      return window.axios
        .get(API_URL + "documents/" + category_id + "/?_page=" + page + "&_per_page=" + perPage)
        .then(response => {
          let data = response.data;
    ...
    fetchSearch({ commit }, [payloads, page, perPage]) {
      return window.axios
        .get(API_URL + "search?q=" + payloads + "&_page=" + page + "&_per_page=" + perPage)
        .then(response => {
          let data = response.data;
